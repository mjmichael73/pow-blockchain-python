...
async def mine(service: BlockchainService = Depends(get_blockchain_service)):
    result = service.mine()
    return {"message": "New block created", **result}
...
async def new_transaction(
    ...
    confirmation_amount: float = Form(...),
    service: BlockchainService = Depends(get_blockchain_service),
):
    ...
    if result is None:
        return JSONResponse(
            status_code=406, content={"message": "Invalid transaction/signature"}
        )
    return JSONResponse(
        status_code=201,
        content={"message": f"Transaction will be added to the Block {result}"},
    )
...
async def register_node(
    nodes: str = Form(...), service: BlockchainService = Depends(get_blockchain_service)
):
    node_list = nodes.replace(" ", "").split(",")
    if not node_list:
        return JSONResponse(
            status_code=400,
            content={"message": "Error: Please supply a valid list of nodes"},
        )
